...
The project's creator (pseudonym: Prometheus) uses Tor/VPN for anonymous accounts, but documents openly (see the project's development log in `historial/`) that account creation for some services happened without Tor/VPN in isolated instances, and that payment methods for infrastructure carry inherent traceability risk if a formal legal request were ever made to the payment provider. This isn't hidden — it's acknowledged as the weakest link in the anonymity chain.
...
El creador del proyecto (seudónimo: Prometheus) usa Tor/VPN para las cuentas anónimas, pero documenta abiertamente (ver el historial de desarrollo en `historial/`) que la creación de cuentas en algunos servicios ocurrió sin Tor/VPN en instancias puntuales, y que el método de pago de la infraestructura conlleva un riesgo de rastreo inherente si alguna vez hubiera una solicitud legal formal al proveedor de pago. No está oculto — se reconoce como el eslabón más débil de la cadena de anonimato.
